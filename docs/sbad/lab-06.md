...
1. **Add the tables.** `app_user` with a unique email and a `password_hash`, and `app_user_role`.
   Seed at least one customer, one staff member and one admin.
...
3. **Show why a fast hash is wrong.** In `docs/security.md`, time BCrypt against SHA-256 for
   10 000 hashes and record both. Explain what the difference buys you.

4. **Implement `UserDetailsService`** loading from the database, with the same vague message for
   an unknown user as for a bad password.
...
8. **Configure the chain.** Stateless, CSRF disabled with a comment saying why that is safe
   here, `/api/auth/**` permitted, everything else authenticated.

9. **Implement `POST /api/auth/login`** returning a token, and one generic error for every
   failure.
...
- [ ] The chain is stateless and ends with `anyRequest().authenticated()`.
