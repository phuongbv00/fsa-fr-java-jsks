...
| A refund needs an approving clerk and a reason | `RETURN_REQUEST` has a `NOT NULL` reason and a foreign key to `STAFF` |
...
diagrams. The notes are written against **PostgreSQL 16**, and every statement in them is
...
Create a database for the module, and get used to throwing it away:

```bash
createdb orderdesk
psql orderdesk -c "SELECT version();"

# The one you will run most often. A script that cannot survive this is not finished.
...
- **Type the examples.** Every snippet in these notes runs against the OrderDesk schema you
  build in lab 02. Change a value and predict the result before you press enter.
